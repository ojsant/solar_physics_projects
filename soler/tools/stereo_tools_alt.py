...
def load_data(opt):
    ...
    if plot_Vsw or plot_N or plot_T:
        magplasma, meta_magplas = stereo_load(instrument='MAGPLASMA', startdate=startdate, enddate=enddate, 
                            path=file_path, pos_timestamp=pos_timestamp, spacecraft=sc)
    ...
    if resample is not None:
        ...
        if plot_Vsw or plot_N or plot_T:
            df_magplas = resample_df(magplasma, resample_mag) 
        if plot_mag:
    ...
    data["df_magplas_pol"] = magplasma  # !!!!!! un-resampled for polarity plotting 

    opt["channels_list"] = channels_list
...
def make_plot(data, opt):
    ...
    channels_list = opt["channels_list"]
...
#"Electrons", "Protons", "PAD", "Mag angles", "Mag", "V_sw", "N", "T", "Polarity", "HET"]
